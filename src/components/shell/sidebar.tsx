      ...
      <div className="scrollbar-none flex-1 overflow-y-auto pb-2">
        {NAV_GROUPS.map((group) => (
          <div key={group.label} className="mb-1">
            {!collapsed && (
              <p className="text-eyebrow mt-3 mb-1 px-4">{group.label}</p>
            )}
            {collapsed && <div className="bg-line mx-3 my-2 h-px" aria-hidden />}
              ...
              {group.items.map((item) => {
                ...
                const link = (
                  <Link
                    ...
                    className={cn(
                      "group flex items-center gap-2.5 rounded-[var(--radius-sm)] px-2.5 py-1.5",
